...
{{- with $user.options }}
{{ $user.name | quote }} ={{ if $user.pool_mode }} pool_mode={{ $user.pool_mode }}{{end}} {{ if $user.max_user_connections }} max_user_connections={{ $user.max_user_connections }}{{end}}
{{- end }}
{{- end }}
...
{{- with .Values.settings.auth_user }}
auth_user = {{ . }}
{{- end }}
...
pool_mode = {{ .Values.settings.pool_mode | default "session" }}
...
max_client_conn = {{ .Values.connectionLimits.max_client_conn }}

;; Default pool size.  20 is good number when transaction pooling
;; is in use, in session pooling it needs to be the number of
;; max clients you want to handle at any moment
default_pool_size = {{ .Values.connectionLimits.default_pool_size }}

;; Minimum number of server connections to keep in pool.
min_pool_size = {{ .Values.connectionLimits.min_pool_size }}

; how many additional connection to allow in case of trouble
reserve_pool_size = {{ .Values.connectionLimits.reserve_pool_size }}
...
log_connections = {{ .Values.logging.log_connections }}

;; log if and why connection was closed
log_disconnections = {{ .Values.logging.log_disconnections }}

;; log error messages pooler sends to clients
log_pooler_errors = {{ .Values.logging.log_pooler_errors }}

;; write aggregated stats into log
log_stats = {{ .Values.logging.log_stats }}

;; Logging verbosity.  Same as -v switch on command line.
verbose = {{ .Values.logging.verbose }}
